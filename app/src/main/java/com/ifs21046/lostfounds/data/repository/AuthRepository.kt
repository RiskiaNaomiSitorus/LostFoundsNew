...
class AuthRepository private constructor(
    ...
    fun register(name: String, email: String, password: String): Flow<MyResult<String>> = flow {
    ...
    fun login(email: String, password: String): Flow<MyResult<UserModel>> = flow {
        emit(MyResult.Loading)
        try {
            val result = apiService.login(email, password) // Get the result from the service
            emit(MyResult.Success(result)) // Emit the result directly
        } catch (e: HttpException) {
            //get error message
            val jsonInString = e.response()?.errorBody()?.string()
            emit(
    ...
    private fun emit(value: MyResult.Success<Any>) {
